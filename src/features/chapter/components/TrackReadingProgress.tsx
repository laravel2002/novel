...
export const TrackReadingProgress = ({
  ...
  useEffect(() => {
    ...
    if (!hasAutoScrolled.current) {
      let savedProgress = getProgress(storySlug) as unknown as
        | { chapterNum?: number; scrollPercentage?: number }
        | number
        | undefined
        | null;

      // HACK: Fallback đọc từ localStorage trực tiếp nếu context chưa kịp nạp
      if (!savedProgress) {
        try {
          const local = localStorage.getItem("novel-reading-progress");
          if (local) {
            const parsed = JSON.parse(local);
            savedProgress = parsed[storySlug];
          }
        } catch (e) {}
      }
      ...
      // Hỗ trợ link "Đọc tiếp" truyền phần trăm cuộn qua ?scroll=
      const searchParams = new URLSearchParams(window.location.search);
      const urlScrollStr = searchParams.get("scroll");
      const urlScroll = urlScrollStr ? parseFloat(urlScrollStr) : 0;

      if (urlScroll > 0) {
        savedPercentage = urlScroll;
      }

      // Chỉ cuộn nếu nó cùng chương được truy cập (hoặc cuộn từ link ?scroll=)
      if ((savedChapter === chapterNum && savedPercentage > 0) || urlScroll > 0) {
    ...
    const handleScroll = () => {
      ...
      lastPercentage.current = clampedPercentage;
    ...
    return () => {
      ...
      // Lưu lại tiến độ ngay khi rời khỏi trang (unmount)
      updateProgress(
        storySlug,
        chapterNum,
