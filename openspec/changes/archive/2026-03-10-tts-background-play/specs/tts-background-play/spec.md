...
- **THEN** nó PHẢI được gán vào một biến toàn cục (ví dụ: `window.currentUtterance`) để ngăn chặn trình dọn rác bộ nhớ của Chrome xóa đối tượng này khi nó vẫn đang phát thanh. Lệnh này đã có trên phiên bản hiện tại, chỉ cần đảm bảo duy trì trong bản refactor.
