...
export function ChapterContent({
  ...
  return (
    <>
      <div className="prose dark:prose-invert max-w-5xl mx-auto mb-12 bg-card shadow-lg rounded-xl border p-12 transition-all duration-300">
        ...
        >
          ...
          {paragraphs.map((paragraph, index) => (
            ...
            >
              <p className="leading-relaxed rounded-md px-2 -mx-2 transition-colors duration-300">
                {paragraph}
              </p>

              {/* Nút hiện bình luận ở đầu đoạn hoặc bên phải */}
              <div className="absolute top-0 -right-12 h-full hidden group-hover:flex items-start opacity-0 group-hover:opacity-100 transition-opacity">
                <Button
                  size="icon"
                  variant="ghost"
                  className="rounded-full w-8 h-8 text-muted-foreground hover:bg-primary/10 hover:text-primary z-10 shadow-sm"
                  onClick={() => {
                    setActiveCommentParaId(index);
                    setCommentOpen(true);
                  }}
