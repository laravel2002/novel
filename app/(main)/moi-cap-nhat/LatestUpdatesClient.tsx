...
export function LatestUpdatesClient({ initialData }: { initialData: any }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const res = await fetchStoriesPaginated({
                limit: 12,
                cursor: parsedCursor,
                sortBy: "updatedAt",
            });
        ...
        fetchData();
    }, [cursorParam, initialData]);

    const { data: stories, nextCursor } = data;
                    ...
                    >
                        {cursorParam ? (
                            <Link href="/moi-cap-nhat">
                                <IconChevronLeft className="w-5 h-5 mr-3 opacity-70" /> Về Mới Nhất
                            </Link>
                        ) : (
